...
nomred <- nomred[nomred$CodiGeo==40302,]
...
quick <- function(quadre) {
  ...
  instr <- afegeix(instr,c("LAST", "Dca", 
                           cometes(paste("cementiri",
                                         a(fila$llocLabel)))))  
  instr <- afegeix(instr, c("LAST", "P31", "Q39614", "S248", "Q98463667"))  
  ...
  instr <- afegeix(instr, c("LAST", "Den", 
                            paste0('"cemetery in ', fila$llocLabel,
                                   " (Catalonia)",'"')))  
  if (nrow(quadre)>1) {
